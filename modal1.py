...
class BiLSTMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x: (B, L, D)
        Returns:
            output: (B, L, D) - same dimension as input
        """
...
class SwinTransformerEncoder(nn.Module):
    """Enhanced Swin Transformer Encoder with BiLSTM"""
    def __init__(self, img_size=(64, 256), patch_size=4, in_chans=1, embed_dim=128,
                 depths=[2, 2, 6], num_heads=[4, 8, 16],
                 window_size=4, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0.1, attn_drop_rate=0.1, output_dim=768):
        super().__init__()
        ...
        self.bilstm = BiLSTMLayer(final_dim, final_dim // 2, num_layers=2, dropout=0.3)
...
class ViLanOCR_Hindi(nn.Module):
    """Enhanced Vision Language OCR for Hindi"""
    def __init__(self, vocab_size=VOCAB_SIZE, d_model=768, max_seq_len=50,
        ...
        # Enhanced Swin Transformer with BiLSTM
...
class CursiveAugmentedDataset(Dataset):
    """Enhanced dataset with more aggressive augmentations"""
    ...
    def augment_image(self, img):
        ...
        # Gaussian blur (simulate poor image quality)
...
class ViLanOCRTrainer:
    ...
    def __init__(self, model, device='cuda', label_smoothing=0.1, ctc_weight=0.3, use_focal_loss=False):
        ...
        if use_focal_loss:
            self.ce_criterion = FocalLoss(alpha=0.25, gamma=2.0, ignore_index=PAD_IDX)
        else:
    ...
    def train_epoch(self, dataloader, optimizer, use_amp=False):
        self.model.train()
        total_loss = 0
        total_ce_loss = 0
        total_ctc_loss = 0
...
if __name__ == "__main__":
    ...
    BATCH_SIZE = 8  # Keep original batch size for stability
    LEARNING_RATE = 0.0001  # Original LR
    NUM_EPOCHS = 6  # Slightly more epochs
    ...
    print("="*60)
    print("Enhanced ViLanOCR: Hindi Handwriting Recognition")
    print("Architecture: Swin Transformer + BiLSTM + mBART + CTC")
    print("="*60)
    ...
    # Initialize model
    print("\nInitializing Enhanced ViLanOCR...")
    model = ViLanOCR_Hindi(vocab_size=VOCAB_SIZE, d_model=768, max_seq_len=50, 
                           img_height=64, img_width=256, use_ctc=True)
    ...
    # Initialize trainer
    trainer = ViLanOCRTrainer(model, device=DEVICE, label_smoothing=0.1, 
                             ctc_weight=0.3, use_focal_loss=False)
    
    # Optimizer with original settings
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_loss, train_ce_loss, train_ctc_loss = trainer.train_epoch(train_loader, optimizer, use_amp=USE_AMP)
